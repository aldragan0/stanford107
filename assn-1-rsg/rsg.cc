...
static void randomSentenceGenerator(std::string start,
                             std::vector<std::string>& sentence,
                             map<std::string, Definition>& grammar,
                             unsigned int ind){
    ...
    std::map<std::string, Definition>::iterator
    grammar_iter = grammar.find(sentence[ind]);

    if(grammar_iter != grammar.end()){
        Production prod = grammar_iter->second.getRandomProduction();
        sentence.erase(sentence.begin() + ind);
        sentence.insert(sentence.begin() + ind, 
                              prod.begin(), prod.end());
    }
    else if(sentence[ind] == ";"){
        sentence[ind] = "";
    } 
    else{
        ++ind;
    }
    randomSentenceGenerator(start, sentence, grammar, ind);
}

/**
  beginning with the start word, replaces one by one non-terminals
  with their corresponding random terminals, until reaching the ennd of
  the "sentence" vector
*/

static void printSentence(map<std::string, Definition>& grammar){
    std::vector<std::string> sentence;
    randomSentenceGenerator("<start>", sentence, grammar, 0);
...
/**
  calls randomSentenceGenerator() to generate a random sentence
  and prints the generated sentence
*/
